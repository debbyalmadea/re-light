...
export const paramsRegex = /(?<![<][^>]*?)(?<!<span[^>]*?>[^<]*?)((\w+)(?==(?:\{[^\}]*\}|<[^>]*?"[^"]*"|<[^>]*?'[^']*'))|(?<=\([^\)]*)(?<!=)(\w+)(?==.*\)))(?!<\/[^>]*>)/g
export const operationRegex = /(?<![<][^>]*?)(?<!<span[^>]*?>[^<]*?)([\+\/\-\*\%\!\=\,\.\:\;\@\\\?\|\^]|&lt;|&gt;|&&)(?!(<\/[^>]*>))/g
export const wordRegex = /(?<![<][^>]*?)(?<!<span[^>]*?>[^<]*?)(\w+)(?!<\/[^>]*>)/g
export const parenthesesRegex = /(?<![<][^>]*?)(?<!<span[^>]*?>[^<]*?)[\(\)\[\]\{\}](?!<\/[^>]*>)/g
...
export const jsxPattern = {
    className: /(?<![<][^>]*?)(?<!<span[^>]*?>[^<]*?)(((?<=(&lt;)\/?.*)(\b[A-Z]\w*\b))|((?<!&gt;)(\w+)(?=&lt;[^\/])(?!.*\())|((?<=(type|as)\s+)(\w+))|((?<=new\s+)(\w+))|((?<!case|default\:\s+)(?<=\w+\:\s+)(\w+)))(?!<\/[^>]*>)/g,
    comment: /(\/\/(.*))|(\/\*([\s\S]*?)\*\/)/g,
    functionName: /(?<![<][^>]*?)(?<!<span[^>]*?>[^<]*?)(((\w+)(?=(&lt;.*&gt;)?\((.*)))|((?<=function\s+)(\w+))|((?<=(?:const|let|var)\s+)(\w+)(?=\s+\=\s+\()))(?!<\/[^>]*>)/g,
    special: /(?<![<][^>]*?)(?<!<span[^>]*?>[^<]*?)(?<=&lt;\/?)(\w+)(?!<\/[^>]*>)/g
}